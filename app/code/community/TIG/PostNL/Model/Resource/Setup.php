...
class TIG_PostNL_Model_Resource_Setup extends Mage_Eav_Model_Entity_Setup
{
    ...
    /**
     * Updates the CIF webservice versions to the newer versions used in PostNL extension version 1.7
     *
     * Deliverydate Webservice:     1_2 -> 2_0
     * Labelling Webservice:        1_6 -> 1_10
     * Location Webservice:         1_1 -> 2_0
     * Timeframe Webservice:        1_2 -> 2_0
     *
     * @return $this
     */
    public function upgradeCifVersions()
    {
        $conn = $this->getConnection();
        ...
        try {
            ...
            $conn->update(
                ...
                array(
                    'path = ?'     => 'postnl/advanced/cif_version_deliverydate',
                )
            );
            $conn->update(
                $this->getTable('core/config_data'),
            ...
            $conn->update(
                ...
                array(
                    'path = ?'     => 'postnl/advanced/cif_version_location',
                )
            );
            $conn->update(
                $this->getTable('core/config_data'),
                ...
                array(
                    'path = ?'     => 'postnl/advanced/cif_version_timeframe',
                )
            );
        } catch (Exception $e) {
            Mage::helper('postnl')->logException($e);
